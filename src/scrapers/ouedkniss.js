...
import { isServicePaused, triggerBackoff, detectRateLimitStatus } from '../config/rateLimiter.js';
 ...
 * @param {number} [options.maxPages] – pages to scrape (default: 1 for free tier)
...
export async function scrapeOuedkniss(query, options = {}) {
  if (isServicePaused('scrapingbee')) return [];

  const { maxPages = 1 } = options;  // reduced from 2 for free tier
  const allListings = [];

  for (let page = 1; page <= maxPages; page++) {
    if (isServicePaused('scrapingbee')) break;
    ...
    try {
      ...
      if (response.status === 429 || response.status === 402) {
        triggerBackoff('scrapingbee', response.status);
        break;
      }
      ...
      if (status) {
        triggerBackoff('scrapingbee', status);
        break;
      }
